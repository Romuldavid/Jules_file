...
# Function to fetch MOEX history
...
# Cash-and-Carry Simulation
initial_capital = 1000000.0
capital = initial_capital

# Tariffs
spot_fee_rate = 0.00025  # 0.025% spot fee (broker + exchange)
...
for secid, name, start_d, end_d in futures_list:
    ...
    # Entry date: first day available
    entry_date = start_d
    if entry_date not in fut_dict or entry_date not in spot_dict:
    ...
    # Exit date: last day available
    exit_date = end_d
    if exit_date not in fut_dict or exit_date not in spot_dict:
        common_dates = sorted(list(set(fut_dict.keys()).intersection(set(spot_dict.keys()))))
        if not common_dates:
            continue
    ...
    # Capital allocation:
    # We use ~75% capital to buy spot yuan (1 lot = 1,000 CNY)
    # 25% kept in cash RUB as margin (GO) for FORTS short futures
    # Since 1 lot spot = 1,000 CNY and 1 fut = 1,000 CNY, number of contracts = number of spot lots
    
    # Price per lot = p_spot_entry * 1000
    # Number of lots = int((capital * 0.75) / (p_spot_entry * 1000))
    ...
    # Futures PnL = (p_fut_entry - p_fut_exit) * cny_amount
    ...
    # Annualized return for this cycle
    ...
    trade_log.append({
        ...
        'fees_rub': spot_fee_entry + spot_fee_exit + forts_fee_entry + forts_fee_exit,
        'net_pnl_rub': net_pnl,
...
print("\n=== CASH AND CARRY BACKTEST RESULTS (2023 - 2025) ===")
print(df_trades[['contract', 'entry_date', 'exit_date', 'days', 'spot_entry', 'fut_entry', 'contango_pct', 'net_pnl_rub', 'annual_return_pct', 'capital_end', 'cbr_rate']].to_string())
