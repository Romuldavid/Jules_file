"""
================================================================================
  БЭКТЕСТ И ГЕНЕРАЦИЯ ОТЧЕТА: LONG STRADDLE С ДИНАМИЧЕСКОЙ СТАВКОЙ ЦБ (НАБОР 2)
  Период: 01.01.2023 — 18.09.2026
  Начальный капитал: 1,000,000.00 руб.
  Инструменты: GZ, VB, GK, SV, GD, BR, NG, CR, RI
================================================================================
"""
...
def run_backtest_and_report():
    print("Загрузка реальных цен MOEX для 9 инструментов (01.01.2023 — 18.09.2026)...")
    futures_data = {}
    for t in TICKERS:
    ...
    fig.suptitle('Результаты стратегии Long Straddle с ДИНАМИЧЕСКОЙ СТАВКОЙ ЦБ (Второй набор активов: MOEX 01.01.2023 - 18.09.2026)', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    chart_path = 'straddle_bond_set2_chart.png'
    ...
    report_md = f"""# Отчет по бескупонной/защищенной опционной стратегии Long Straddle на MOEX (Набор 2: GZ, VB, GK, SV, GD, BR, NG, CR, RI)
...
* **Доля успешных опционных сделок с фиксацией теейк-профита (+20%):** **{overall_tp_rate:.1f}%** ({total_tp_hits} из {total_trades} торговых циклов закрыты с досрочной прибылью).
...
### 1. Лидеры группы: {top_asset_data['Name']} (`{top_asset_code}`), {sec_asset_data['Name']} (`{sec_asset_code}`)
* **{top_asset_data['Name']} (`{top_asset_code}`):** Лидер списка с итоговой прибылью **+{top_asset_data['TotalPnL']:,.2f} ₽ (+{top_asset_data['YieldPct']:.2f}%)**. {top_asset_data['TPHits']} из {top_asset_data['Trades']} циклов закрыты досрочно по тейк-профиту +20%.
* **{sec_asset_data['Name']} (`{sec_asset_code}`):** Второе место с чистой прибылью **+{sec_asset_data['TotalPnL']:,.2f} ₽ (+{sec_asset_data['YieldPct']:.2f}%)**.
...
1. **100% Защита капитала:** Прибыль стратегии составила **+{total_yield_pct:.2f}%** (**{total_final_capital:,.2f} ₽**).
2. **Нулевая просадка:** Никакой риск для тела первоначального депозита отсутствовал.
"""

    report_path = 'straddle_bond_set2_report.md'
