  ...
  "cells": [
    {
      ...
      "metadata": {},
      "source": [
        "# 🇨🇳📊 Валютный Cash-and-Carry на Московской бирже (MOEX): Бэктест 2023–2025 гг.\n",
        "\n",
        "**Автор:** Jules / MOEX Quant Trading Desk  \n",
        "**Начальный капитал:** `1,000,000 рублей`  \n",
        "**Период анализа:** `01 января 2023 года — Настоящее время`  \n",
        "**Данные:** Только реальные исторические и текущие цены MOEX ISS API (`CNYRUB_TOM` и фьючерсы `CR` / `CNY-`)\n",
        "\n",
        "---\n",
        "\n",
        ...
        "2. **Одновременной продаже (шорте) фьючерсного контракта** на этот же актив на срочном рынке FORTS (квартальные фьючерсы `CRH3`, `CRM3`, `CRU3`, `CRZ3`, `CRH4`, `CRM4` и т.д.).\n",
        "\n",
        ...
        "\n",
        "### 🛡️ Управление риском бэквордации (Санкции 2024 года):\n",
        "В июне 2024 года после введения санкций США на MOEX и НКЦ возник локальный дефицит юаней, из-за чего фьючерсы временно ушли в **бэквордацию** (цена фьючерса ниже спота).  \n",
        "**Умное правило алгоритма (Smart Filter):**\n",
        "- Если рынок находится в контанго (`Контанго > 0.5%`) — алгоритм исполняет **Cash-and-Carry**.\n",
        "- Если рынок находится в бэквордации — алгоритм размещает капитал в **Денежный рынок MOEX (СВОП / RUSFAR)**, зарабатывая высокую ключевую ставку ЦБ (16%–21% годовых).\n",
        "\n",
        "---\n",
        "\n",
        "## 💰 Тарифы и Залоговое Обеспечение (ГО):\n",
        "- **Лотность:** 1 лот спота `CNYRUB_TOM` = 1,000 CNY. 1 контракт фьючерса `CR` = 1,000 CNY. Хедж 1:1.\n",
        "- **Комиссия спот-рынка:** `0.025%` от суммы сделки (Биржа + Брокер Тариф «Срочный / Валютный»).\n",
        "- **Комиссия FORTS:** `2.50 руб.` за 1 контракт фьючерса (Биржа + Брокер).\n",
    ...
    },
    {
      ...
      "metadata": {},
      "source": [
        "## 🏛️ 2. Таблица Изменений Ключевой Ставки ЦБ РФ (2023–2025 гг.)\n",
        "Ключевая ставка Банка России является фундаментом для расчета процентного контанго во фьючерсах."
      ]
    },
    {
      "cell_type": "code",
      ...
      "outputs": [],
      "source": [
        "# Хронология изменений ключевой ставки ЦБ РФ с 01.01.2023\n",
        "cbr_rates_history = [\n",
        ...
        "    ('18.12.2023 - 28.07.2024', 16.00, 'Удержание высокой ставки'),\n",
        "    ('29.07.2024 - 15.09.2024', 18.00, 'Новый этап повышения'),\n",
        "    ('16.09.2024 - 27.10.2024', 19.00, 'Борьба с перегревом экономики'),\n",
        "    ('28.10.2024 - н.в.', 21.00, 'Рекордный уровень ключевой ставки')\n",
        "]\n",
        "\n",
    ...
    },
    {
      ...
      "metadata": {},
      "source": [
        "## 📥 3. Загрузка реальных исторических данных MOEX ISS API и Симуляция Бэктеста"
      ]
    },
    {
      "cell_type": "code",
      ...
      "source": [
        ...
        "\n",
        "print('=== ИТОГОВАЯ ТАБЛИЦА ВСЕХ СДЕЛОК C 01.01.2023 ПО 2025 ГОД ===')\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "ax1.set_title('Динамика Капитала (1,000,000 руб.) на Валютном Cash-and-Carry MOEX', fontsize=14, fontweight='bold', pad=12)\n",
        "ax1.set_ylabel('Капитал (рубли)', fontsize=11)\n",
        "ax1.yaxis.set_major_formatter('{x:,.0f}')\n",
        "ax1.grid(True, linestyle='--', alpha=0.5)\n",
        "\n",
        "for _, row in df_trades.iterrows():\n",
        "    ax1.annotate(f\"+{row['Чистый PnL (руб)']:,.0f} ₽\\n({row['Доходность (APR %)']:.1f}% APR)\", \n",
        "                 (pd.to_datetime(row['Дата выхода']), row['Капитал (руб)']),\n",
        "                 textcoords=\"offset points\", xytext=(0,10), ha='center', fontsize=8, fontweight='bold', color='#2B6CB0')\n",
        "\n",
        "# График 2: Ключевая ставка ЦБ\n",
        "ax2.step(df_equity['date'], df_equity['Ставка ЦБ (%)'], where='post', color='#C53030', linewidth=2, label='Ключевая ставка ЦБ РФ (%)')\n",
        ...
        "print(f\"🏁 Конечный капитал: {capital:,.2f} руб.\")\n",
    ...
    },
    {
      ...
      "metadata": {},
      "source": [
        "## 📡 5. Модуль Мониторинга и Анализа Рынка в Реальном Времени (24/7 Live Advisory)\n",
        "Подключается к котировкам MOEX ISS API прямо сейчас и вычисляет текущий арбитражный спред между `CNYRUB_TOM` и активным фьючерсом на юань!"
      ]
    },
    {
      "cell_type": "code",
      ...
      "outputs": [],
      "source": [
        "def check_live_cny_arbitrage():\n",
        "    print('=== ПРОВЕРКА ТЕКУЩЕГО АРБИТРАЖНОГО СПРЕДА С ЦЕНТРАЛЬНОГО СЕРВЕРА MOEX ===')\n",
        "    try:\n",
        "        # 1. Get Spot CNY price\n",
        "        r_spot = requests.get('https://iss.moex.com/iss/engines/currency/markets/selt/boards/CETS/securities/CNYRUB_TOM.json').json()\n",
        "        spot_data = r_spot['marketdata']['data']\n",
        "        spot_cols = r_spot['marketdata']['columns']\n",
        "        df_s = pd.DataFrame(spot_data, columns=spot_cols)\n",
        "        spot_last = df_s['LAST'].dropna().iloc[0] if not df_s['LAST'].dropna().empty else df_s['MARKETPRICE'].dropna().iloc[0]\n",
        "        \n",
        "        # 2. Get active futures\n",
        "        r_fut = requests.get('https://iss.moex.com/iss/engines/futures/markets/forts/securities.json').json()\n",
        "        f_cols = r_fut['securities']['columns']\n",
        "        df_f = pd.DataFrame(r_fut['securities']['data'], columns=f_cols)\n",
        "        active_cny_futs = df_f[df_f['SHORTNAME'].str.contains('CNY-', case=False, na=False) & ~df_f['SHORTNAME'].str.contains('UCNY', case=False, na=False)]\n",
        "        \n",
        "        if active_cny_futs.empty:\n",
        "            print('Не найдены активные контракты фьючерсов.')\n",
        "            return\n",
        "            \n",
        "        first_fut = active_cny_futs.iloc[0]\n",
        "        secid = first_fut['SECID']\n",
        "        shortname = first_fut['SHORTNAME']\n",
        "        fut_last = first_fut['PREVPRICE'] if pd.isna(first_fut['LASTSETTLEPRICE']) else first_fut['LASTSETTLEPRICE']\n",
        "        \n",
        "        contango_pct = ((fut_last / spot_last) - 1) * 100\n",
        "        ann_contango = contango_pct * 4.0 # Approx 3 months hold\n",
        "        \n",
        "        print(f\"📍 Спот юань (CNYRUB_TOM): {spot_last:.4f} руб.\")\n",
        "        print(f\"📍 Ближайший фьючерс ({secid} / {shortname}): {fut_last:.4f} руб.\")\n",
        "        print(f\"📊 Текущий базис (Контанго): {contango_pct:+.2f}% (Годовая ставка: {ann_contango:+.2f}% годовых)\")\n",
        "        \n",
        "        if contango_pct > 0.5:\n",
        "            print('👉 РЕКОМЕНДАЦИЯ: Входить в Cash-and-Carry (Покупка спот CNYRUB_TOM + Шорт фьючерса)!')\n",
        "        else:\n",
        "            print('👉 РЕКОМЕНДАЦИЯ: Рынок в бэквордации. Размещать рубли в RUSFAR / СВОП под ставку ЦБ!')\n",
        "            \n",
        "    except Exception as e:\n",
        "        print('Ошибка получения live котировок:', e)\n",
        "\n",
        "check_live_cny_arbitrage()"
      ]
    }
  ],
  "metadata": {
...
with open("cny_cash_and_carry_backtest.ipynb", "w", encoding="utf-8") as f:
    json.dump(notebook, f, ensure_ascii=False, indent=2)

print("Colab notebook cny_cash_and_carry_backtest.ipynb created successfully.")
