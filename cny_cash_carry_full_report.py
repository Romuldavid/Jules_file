...
================================================================================
  ...
  Период: 01 января 2023 г. — 2025 г.
  Начальный капитал: 1 000 000.00 рублей
  Инструменты: Спот юань (CNYRUB_TOM) + Квартальные фьючерсы (CRH3 - CRH5)
...
# ==============================================================================
# 1. ТАБЛИЦА ИЗМЕНЕНИЙ КЛЮЧЕВОЙ СТАВКИ ЦБ РФ (2023 - 2025 гг.)
# ==============================================================================
CBR_RATES_SCHEDULE = [
    ...
    ('2024-10-28', '2025-12-31', 21.00, 'Рекордный уровень ставки 21%')
]
...
# ==============================================================================
# 2. ПОЛУЧЕНИЕ ИСТОРИЧЕСКИХ ДАННЫХ С СЕРВЕРА MOEX ISS
# ==============================================================================
...
# ==============================================================================
# 3. ОСНОВНОЙ МОДУЛЬ РАСЧЕТА БЭКТЕСТА И ТАРИФОВ FORTS
# ==============================================================================
def run_full_cny_cash_and_carry_backtest(initial_capital=1000000.0):
    print("=" * 80)
    print("  ЗАПУСК СИМУЛЯЦИИ CASH-AND-CARRY НА MOEX (2023 - 2025)")
    print(f"  Начальный капитал: {initial_capital:,.2f} руб.")
    print("=" * 80)
    
    # Загружаем данные спот юаня
    ...
    # Параметры комиссий
    SPOT_FEE_RATE = 0.00025          # 0.025% (комиссия Валютного рынка MOEX + Брокер)
    FORTS_FEE_PER_CONTRACT = 2.50   # 2.50 руб. за 1 контракт (Комиссия FORTS + Брокер)
    ...
    for secid, short_name, start_date, end_date in futures_contracts:
        ...
        # SMART FILTER: Входим в Cash-and-Carry при положительном контанго (>0.5%).
        # При бэквордации (санкционный период 2024 г.) размещаем капитал в Денежный рынок MOEX (RUSFAR).
        if contango_pct > 0.5:
            strategy_type = "CASH & CARRY"
            # 75% капитала выделяется на покупку спот-юаня, 25% остается на ГО фьючерса
    ...
    print(f"  Конечный депозит:  {current_capital:,.2f} руб.")
